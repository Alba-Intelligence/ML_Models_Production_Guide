---
title: "Floci AWS Emulator"
---

This page is the copy-pasteable source for the shared local AWS emulator.
It replaces the earlier LocalStack/MinIO split and is the only local AWS
emulator the repo now treats as canonical.

```{mermaid}
graph LR
  dev[docker-compose.dev.yml] --> floci[Floci]
  infra[docker-compose.local-infra.yml] --> floci
  floci --> bootstrap[floci_bootstrap]
  bootstrap --> buckets[S3 buckets and IAM role]
```
