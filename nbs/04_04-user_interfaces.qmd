...
This notebook describes the design for a centralized notebook repository web UI that enables users across all roles to browse, select, and trigger notebook executions across environments while keeping notebooks as the immutable source of truth.
...
- Role-based access control for notebook access and execution triggering, with the UI available to all user roles for the actions they are authorized to perform
...
5. Add role-based access control for all user roles
6. Integrate MCP tool launching from execution results for authorized roles
