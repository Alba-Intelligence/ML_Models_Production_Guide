...
def render_local_infra_compose_config(
    ...
    """Render local emulation compute-plane config (AWS APIs, Kubernetes, Slurm)."""
    runtime = config or LocalInfrastructureParityConfig()
    return {
        "services": {
            "localstack": {
                "image": "localstack/localstack:3",
                "ports": [f"{runtime.localstack_port}:4566"],
                "environment": {
                    "SERVICES": "s3,iam,ec2,sts,secretsmanager,cloudwatch,logs",
                    "DEBUG": "0",
